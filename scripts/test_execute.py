...
def phase_dir(tmp_project):
    ...
    index = {
        "project": "TestProject",
        "phase": "mvp",
        "rules": ["coding-style", "karpathy-guidelines", "security"],
        "steps": [
...
def executor(tmp_project, phase_dir):
    ...
    with patch.object(ex, "TEMPLATE_ROOT", tmp_project):
...
class TestLoadGuardrails:
    ...
    def test_empty_project(self, tmp_path):
        with patch.object(ex, "TEMPLATE_ROOT", tmp_path):
...
class TestMainCli:
    def test_no_args_exits(self):
        with patch("sys.argv", ["execute.py"]):
            with pytest.raises(SystemExit) as exc_info:
                ex.main()
            assert exc_info.value.code == 1  # prints help and exits 1

    def test_invalid_phase_dir_exits(self):
        with patch("sys.argv", ["execute.py", "execute", "nonexistent"]):
            with pytest.raises(SystemExit) as exc_info:
                ex.main()
            assert exc_info.value.code == 1  # StepExecutor exits 1

    def test_missing_index_exits(self, tmp_project):
        (tmp_project / "phases" / "empty").mkdir()
        with patch("sys.argv", ["execute.py", "execute", "empty"]):
            with patch.object(ex, "TEMPLATE_ROOT", tmp_project):
                with pytest.raises(SystemExit) as exc_info:
                    ex.main()
                assert exc_info.value.code == 1
